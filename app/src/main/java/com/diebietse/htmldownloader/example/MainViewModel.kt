...
import com.diebietse.htmldownloader.DefaultFileSaver
import com.diebietse.htmldownloader.HtmlDownloader
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    suspend fun download(url: String) = withContext(Dispatchers.IO) {
        ...
        HtmlDownloader().download(url, DefaultFileSaver(File(downloadDir, pageId)))
        prefs.edit().putString(pageId, url).apply()
    }
